                ...
                <Card>
                    <CardContent className="flex flex-col xl:flex-row gap-4 justify-between items-start xl:items-end">
                        <div className="flex items-end gap-3 flex-wrap w-full xl:w-auto">
                            ...
                            {(dateFrom || dateTo || filterUserId || statusFilter || searchTerm) && (
                                <Button
                                    variant="ghost"
                                    size="sm"
                                    className="h-9 text-xs text-muted-foreground ml-1"
                                    onClick={() => { setDateFrom(undefined); setDateTo(undefined); setFilterUserId(''); setStatusFilter(''); setSearchTerm(''); setCurrentPage(1); }}
                                >
                                    <X className="w-3 h-3 flex-shrink-0 mr-1" /> Limpiar
                                </Button>
                            )}
                        </div>
                                ...
                                <Popover>
                                    <PopoverTrigger asChild>
                                        <Button
                                            variant={"outline"}
                                            className={cn(
                                                "w-full sm:w-[160px] h-9 justify-start text-left font-normal text-sm flex-row items-center bg-background",
